...
const DayAccordion = ({ day, tasks, excludedKeys, onApprove }) => {
    ...
    const approveAndAllocateVehicle = async (workplanId) => {
        try {
            ...
            // Prompt the user to enter a vehicle_name and pilot_name
            const vehicle_name = prompt('VEHICLE NAME');
            const pilot_name = prompt('PILOT NAME');

            if (!vehicle_name && !pilot_name) {
                return;
            }

            // Make a PUT request to your backend API endpoint to approve the workplan
            await axios.put(`${url}/workplans/allocate/${workplanId}`, { Workplan_id: workplanId, vehicle_name: vehicle_name, pilot_name: pilot_name });
                                    ...
                                    {unitTasks.map((task, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className='text-xs'>
                                                <button className='btn btn-success btn-sm' onClick={() => approveAndAllocateVehicle(task.workplan_id)}>Assign</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
