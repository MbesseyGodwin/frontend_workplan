        ...
        <div className="grid grid-cols-3 md:grid-cols-4 gap-2">
          {dashboardLinks.map((link, index) => (
            ...
            loggedInUser.roleID !== 1 && link.title === "manage users" ? null : (
              <Link key={index} to={`../${link.url}`} className="text-center text-dark align-center py-3 px-0 ease-in duration-500 bg-slate-300 shadow rounded-lg no-underline hover:bg-gray-400 hover:text-white">
                {link.icon && React.cloneElement(link.icon, { style: { color: '#912222', fontSize: 45 } })}
                <h3 className="text-xs lg:text-xl my-2 mx-0 font-semibold capitalize">{link.title}</h3>
              </Link>
            )
          ))}
        </div>
